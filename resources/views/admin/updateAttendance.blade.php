...
<div class="w-full h-5/6">
    <form method="GET" action="{{ route('admin.updateAttendance') }}" class="w-full h-2/6">
        ...
        <div class="relative w-full max-w-xl mx-auto pl-24 bg-white rounded-full justify-center">
            ...
            <div class="relative max-w-sm">
                ...
                <input datepicker id="default-datepicker" type="text" name="date"
                    ...
                    placeholder="Select date" value="{{ request()->get('date', date('Y-m-d')) }}">
            </div>
        </div>
    </form>
    ...
    <div class="flex flex-col w-full justify-center items-center mt-4">
                        ...
                        <table class="w-full lg:w-full">
                            ...
                            <tbody class="divide-y divide-gray-300">
                                @foreach ($data as $timetable)
                                    @foreach ($timetable->attendance as $attendance)
                                        <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                                            ...
                                            <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                {{ $attendance->present == 'Yes' ? 'Present' : 'Absent' }}
                                            </td>
                                            <td class="p-5">
                                                <div class="flex items-center gap-1">
                                                    <button
                                                        class="p-2 rounded-full group transition-all duration-500 flex items-center">
                                                        <a href="{{ $attendance->student->role == "student" ? route('StudentForm') : route('TeacherForm') }}"
                                                            class="w-full">
                                                    ...
                                                    <form id="postForm" action="{{ route('Profile') }}" method="POST"
                                                        style="display: none;">
                                                        @csrf
                                                        <input type="hidden" name="id" value="{{ $attendance->student->id }}">
                                                        <input type="hidden" name="role" value="{{ $attendance->student->role }}">
                                                    </form>
        ...
        @else
            <div class="flex justify-center">
                <img src="{{ asset('img/find.jpg') }}" alt="Find" class="w-1/2 h-auto">
            </div>
        @endif
    </div>
</div>
