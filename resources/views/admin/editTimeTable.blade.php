            ...
            @foreach ($courses as $course)
                <li class="group relative text-sm">
                    <button class="outline-none focus:outline-none rounded-md px-3 py-2 bg-white flex items-center w-72"
                        ...
                        <span class="pr-1 font-semibold flex-1">{{ $course->course_name }}</span>
                        <span>
    ...
    <div class="w-full flex flex-row justify-evenly px-7 sticky top-1 mr-32">
        <div class="daysmenu">
            ...
            @foreach($days as $dayName)
                <button
                    class="day-button {{ strtolower($dayName) }} text-green-700 hover:text-white border border-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg h-10 me-2 mb-2"
                    data-day="{{ strtolower($dayName) }}">
                    <a href="javascript:void(0);" data-day="{{ strtolower($dayName) }}"
        ...
        <button
            ...
            <a href="{{ route('addTable', ['day' => $selectedDay]) }}"
                    ...
                    <div
                        ...
                        <div>
                            <button
                                ...
                                <a href="{{ route('updateTable', ['id' => $TimeTable->id]) }}"
                                    class="w-full h-full text-sm px-5 py-2.5 text-center text-green-700 hover:text-white border border-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 rounded-lg">
                                    Edit
                                </a>
                            </button>
                            <form action="{{ route('deleteTable', ['id' => $TimeTable->id]) }}" method="POST"
...
@push('script')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            let selectedDay = '{{ strtolower($selectedDay) }}'; 
