...
<div class="font-bold text-xl text-green-600">
    <span class="font-extrabold">TimeTable - </span>{{ $batchName ?? 'No Classes' }}
</div>
@endsection
...
<div class="flex flex-col items-center w-full">
    <div class="w-full flex flex-row justify-evenly px-7 sticky top-1 mr-32">
        ...
        @foreach($days as $dayName)
            <button
                ...
                <a href="{{ route('studentTimeTable', ['day' => $dayName]) }}"
    ...
    <div
        class="w-full h-5/6 pl-20 content-center justify-center bottom-0 overflow-y-scroll overflow-x-hidden no-scrollbar">
        @if($data->isEmpty())
        ...
        @else
            @foreach ($data as $TimeTable)
                <div class='relative w-5/6 h-28 rounded-xl  overflow-hidden flex flex-col bg-transparent   timetable-item'>
                    <div
                    ...
                    <div class="relative progressBar w-full  h-5 bg-gray-200 mb-4 rounded-xl">
                        <div class="absolute top-0 z-10  h-5 bg-green-500  rounded progress-bar" style="width:0%;">
                        </div>
                    </div>
                </div>
            @endforeach
